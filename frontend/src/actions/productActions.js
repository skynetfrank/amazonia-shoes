...
export const updateExistencia = (item) => async (dispatch, getState) => {
	console.log('ACTIONS UPDATE EXISTENCIA:', item);
	...
	try {
		const { data } = await Axios.put(
			`/api/products/existencia/${item.idProducto}`,
			item,
			{
				headers: { Authorization: `Bearer ${userInfo.token}` },
			}
...
export const getProductoByCode = (codigo) => async (dispatch) => {
	...
	try {
		const { data } = await Axios.get(
			`/api/products/encontrar?codigo=${codigo}`
		);
		dispatch({ type: PRODUCT_BYCODE_SUCCESS, payload: data });
	} catch (error) {
		console.log('actions: getproductByCode ocurrio un error');
		const message =
			error.response && error.response.data.message
				? error.response.data.message
				: error.message;
